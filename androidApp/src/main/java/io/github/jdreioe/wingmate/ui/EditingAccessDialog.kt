    ...
    AlertDialog(
        ...
        title = {
            Text(stringResource(when (mode) {
                EditingAccessDialogMode.Unlock -> R.string.editing_access_unlock_title
                EditingAccessDialogMode.Configure -> R.string.editing_access_setup_title
                EditingAccessDialogMode.Disable -> R.string.editing_access_disable_title
            }))
        },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(stringResource(R.string.editing_access_code_help))
                OutlinedTextField(
                    value = code,
                    onValueChange = { if (it.length <= 8 && it.all(Char::isDigit)) code = it },
                    label = { Text(stringResource(R.string.editing_access_code)) },
                ...
                if (mode == EditingAccessDialogMode.Configure) {
                    OutlinedTextField(
                        ...
                        label = { Text(stringResource(R.string.editing_access_confirm_code)) },
                        visualTransformation = PasswordVisualTransformation(),
                        keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.NumberPassword),
                        singleLine = true,
                        modifier = Modifier.fillMaxWidth()
        ...
        confirmButton = {
            val incorrect = stringResource(R.string.editing_access_incorrect)
            TextButton(
                enabled = canSubmit,
                onClick = {
                    working = true
