...
fun AddCategoryDialog(existingNames: List<String> = emptyList(), availableLanguages: List<String> = emptyList(), onDismiss: () -> Unit, onSave: (CategoryItem) -> Unit) {
    ...
    val nameEmptyMessage = stringResource(R.string.category_error_name_empty)
    val categoryExistsMessage = stringResource(R.string.category_error_exists)
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text(stringResource(R.string.category_add_title)) }, text = {
        Column(modifier = Modifier.fillMaxWidth()) {
            ...
            OutlinedTextField(value = name, onValueChange = {
                name = it
                error = null
            }, label = { Text(stringResource(R.string.category_name_label)) }, modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp).then(showKeyboard))
            if (availableLanguages.isNotEmpty()) {
                ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
                    TextField(value = selectedLanguage, onValueChange = {}, readOnly = true, label = { Text(stringResource(R.string.common_language)) }, trailingIcon = { ExposedDropdownMenuDefaults.TrailingIcon(expanded = expanded) })
