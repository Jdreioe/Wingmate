...
fun VoiceSelectionFullScreen(onNext: () -> Unit, onCancel: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            ...
            val langs = voices.flatMap {
                listOfNotNull(it.primaryLanguage) + (it.supportedLanguages ?: emptyList())
            }.distinct().sorted()
        ...
        Column(modifier = Modifier.fillMaxSize().padding(24.dp)) {
            ...
            if (loading) {
                            ...
                            supportedLanguages.forEach { lang ->
                                ...
                                AssistChip(onClick = {
                                    ...
                                    scope.launch {
                                        try {
                                            val currentSelected =
                                                runCatching { useCase.selected() }.getOrNull()
                                            if (newSel != null) {
                                                if (currentSelected != null) {
                                                    useCase.select(
                                                        currentSelected.copy(
                                                            selectedLanguage = newSel
                                                        )
                                                    )
                                                } else {
                                                    // create a minimal selected voice entry to store the selected language
                                                    val placeholder =
                                                        io.github.jdreioe.wingmate.domain.Voice(
                                                            name = "",
                                                            displayName = "",
                                                            primaryLanguage = newSel,
                                                            selectedLanguage = newSel
                                                        )
                ...
                LazyColumn(modifier = Modifier.fillMaxWidth()) {
                    val filtered = selectedLanguageFilter?.let { f ->
                        voices.filter { v ->
                            (v.primaryLanguage == f) || (v.supportedLanguages?.contains(f) == true)
                        }
                    } ?: voices
                    items(filtered) { v ->
                        Row(
                                ...
                                .clickable {
                                    scope.launch {
                                        try {
                                            ...
                                            val currentSelected =
                                                runCatching { useCase.selected() }.getOrNull()
                                            val effectiveLang = selectedLanguageFilter
                                                ?: currentSelected?.selectedLanguage
                                                ?: v.selectedLanguage.ifBlank {
                                                    v.primaryLanguage ?: ""
                                                }
                    ...
                    VoiceSettingsDialog(
                        ...
                        onSave = { updated ->
                            scope.launch {
                                ...
                                try {
                                    voices =
                                        (useCase.refreshFromAzure() + useCase.list()).distinctBy { it.name }
                                    selected = useCase.selected()
                                } catch (_: Throwable) {
                                }
                            }
