...
class AppDatabase {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2) {
      print('Applying migration to add displayName and locale');
      try {
        await db.execute('ALTER TABLE VoiceItem ADD COLUMN displayName TEXT;');
        await db.execute('ALTER TABLE VoiceItem ADD COLUMN locale TEXT;');
      } catch (e) {
        print('Migration error: $e');
      }
    }
    // Add missing columns for voices if not present
    if (oldVersion < 23) { // Use a new version number for this migration
      print('Applying migration to add pitch, rate, pitchForSSML, rateForSSML, primaryLanguage to VoiceItem');
      try {
        await db.execute('ALTER TABLE VoiceItem ADD COLUMN pitch REAL;');
        await db.execute('ALTER TABLE VoiceItem ADD COLUMN rate REAL;');
        ...
        await db.execute('ALTER TABLE VoiceItem ADD COLUMN primaryLanguage TEXT;');
      } catch (e) {
        print('Migration error: $e');
      }
    }
