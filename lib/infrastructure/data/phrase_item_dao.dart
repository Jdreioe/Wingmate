import 'package:wingmate/infrastructure/data/app_database.dart'; // Import your database helper
...
class PhraseItemDao implements PhraseItemRepository {

  @override
  Future<List<PhraseItem>> getPhraseItems() async {
    return getAll();
  }
  ...
  Future<int> insertPhrase(PhraseItem phraseItem) async {
    final db = await _database.database;
  ...
  Future<int> updatePhrase(PhraseItem phraseItem) async {
    final db = await _database.database;
  ...
  Future<void> updatePhrasePositions(List<PhraseItem> items) async {
    final db = await _database.database;
