...
struct PredictionBar: View {
    ...
    // When true, the bar always occupies its full height even with no
    // predictions so surrounding layout doesn't jump as results arrive/clear.
    var alwaysReserveSpace: Bool = false
    ...
    private var isEmpty: Bool {
        result.words.isEmpty && result.letters.isEmpty
    }
    
var body: some View {
        if !alwaysReserveSpace && isEmpty {
            EmptyView()
        } else {
            VStack(spacing: 0) {
                Divider()
