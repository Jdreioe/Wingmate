...
class AndroidZipArchiveReader(private val file: File) : ArchiveReader {
    private val zip = try {
    ...
    override suspend fun entries(): List<ArchiveEntry> = withContext(Dispatchers.IO) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            result += ArchiveEntry(entry.name, entry.size, entry.compressedSize, entry.isDirectory, entry.generalPurposeBit.usesEncryption())
        }
        result
    }
