...
# Fail if CI_WORKSPACE not set (safety check for running outside Xcode Cloud)
...
# === JDK Setup ===
# Xcode Cloud requires JDK 21 for Kotlin Multiplatform
if ! /usr/libexec/java_home -v 21 &>/dev/null; then
  ...
  # Register for java_home without sudo by setting JAVA_HOME directly
  export JAVA_HOME="$(brew --prefix)/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home"
else
  export JAVA_HOME="$(/usr/libexec/java_home -v 21)"
fi
echo "JAVA_HOME=$JAVA_HOME"
# Ensure Gradle uses our JDK
